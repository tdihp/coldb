...
def c_plain(col_type, col, **kw):
    """ the default non-compression approach """
    if col_type in ('b', 'B', 'h', 'H', 'i', 'I'):
        return c_plain_normal(col_type, col, **kw)
    elif re.match(r'^\d+s$', col_type):
        return c_plain_struct(col_type, col, **kw)
    elif col_type  in ('blob1', 'blob2', 'blob4'):
        return c_plain_blob(col_type, col, **kw)


def c_plain_normal(col_type, col, **kw):
...
def c_plain_struct(col_type, col, **kw):
...
def c_plain_blob(col_type, col, bpt, **kw):
...
def c_run0(col_type, col, pt, **kw):
    if not col_type in ('b', 'B', 'h', 'H', 'i', 'I'):
        raise CompressError("col type %s cannot do run0" % col_type)
    rowlist, vallist = run(0, col)
    rowcnt = len(rowlist)
    result = array_packing((pt, [rowcnt] + rowlist),
                           (col_type, vallist))
    return result


def c_run1(col_type, col, pt, **kw):
    if not col_type in ('b', 'B', 'h', 'H', 'i', 'I'):
